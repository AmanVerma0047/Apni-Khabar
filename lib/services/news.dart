...
class News {
  ...
  // List of sources from NewsAPI: https://newsapi.org/sources
  ...
  /// Picks a random news source from the list.
  String getRandomSource() {
    return sources[_random.nextInt(sources.length)];
  }

  Future<void> getNews() async {
    String selectedSource = getRandomSource();

    String url =
        "https://newsapi.org/v2/top-headlines?sources=$selectedSource&apiKey=$NEWS_API_KEY";
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['status'] == 'ok') {
          news.clear(); // Clear previous articles
          jsonData['articles'].forEach((element) {
            if (element['urlToImage'] != null && element['description'] != null) {
              ArticleModel articleModel = ArticleModel(
                title: element['title'],
